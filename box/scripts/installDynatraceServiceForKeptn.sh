...
export DT_TENANT=$(cat ~/creds.json | jq .dynatraceTenant)
export DT_API_TOKEN=$(cat ~/creds.json | jq .dynatraceApiToken)
export DT_PAAS_TOKEN=$(cat ~/creds.json | jq .dynatracePaasToken)
