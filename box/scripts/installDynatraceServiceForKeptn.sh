...
export DT_TENANT=$(jq -r .dynatraceTenant ~/creds.json)
export DT_API_TOKEN=$(jq -r .dynatraceApiToken ~/creds.json)
export DT_PAAS_TOKEN=$(jq -r .dynatracePaasToken ~/creds.json)
